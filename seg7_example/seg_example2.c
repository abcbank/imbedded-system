...
int main(int argc, char **argv)
{
    ...
    int tmp_n;
    int delay_time;
    int dev = open("/dev/my_segment", O_RDWR); // if you want read-10_RDONLY write="0_WRONLY", read&write='O_RDWR
    ...
    while(1){
        ...
        write(dev, &data[tmp_n], 2); usleep (delay_time);
        tmp_n++;
    ...
    close_keyboard(); write(dev, 0x0000, 2);
    close(dev);
    return 0;
}